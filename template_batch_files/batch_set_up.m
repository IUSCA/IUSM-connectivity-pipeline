...
% Example of data structure for parcellation input.
...
parcs.plabel(1).name='shen_286';
parcs.pdir(1).name='shen_MNI152_v1_5';
...
flags.T1.anat = 1; % run FSL_anat 
    ...
    configs.T1.crop = 1; % 0 = no; 1 = yes (lots already done by dcm2niix)
flags.T1.bet = 1; % brain extraction and mask generation
    configs.T1.betF = 0.5;  % These are brain extraction parameters within FSL bet.
    configs.T1.betG = 0; % See fsl bet help page for more details.   
...
flags.T1.reg2MNI=1;
    ...
    configs.T1.fnirtSubSamp='4,4,2,2'; %subsampling for fnirt registration to MNI; default string = '4,4,2,2'
flags.T1.seg = 1;
...
flags.EPI.UseJson = 0; % obtain pertinent scan information through json files generated by dcm2niix
flags.EPI.SpinEchoUnwarp = 1;% Requires UNWARP directory and approporiate dicoms.
flags.EPI.UseUnwarped = 1; % Use unwarped EPI if both warped and unwarped are available.
...
flags.EPI.RunTopup = 1; % 1 = Run topup (1st pass), 0 = Do not rerun if previously completed.       
...
% the 75th percentile + 1.5 times the interquartile range.
    configs.EPI.FDth = 0.30;
    configs.EPI.DVARSth = 1.7;  % Column 1 only output
...
flags.EPI.TissueRegressors = 1;
    configs.EPI.numCompsPCA = 5;% 0 1 3 5 will be tested (No more than 5)
...
flags.DWI.genMats = 1; % generate connectivity matrices
...
%%
                    ...
                    %------------------------------%  
                       ...
                       %--------------------------%
flags.DWI.bedpostXprep = 1; % create directory and links for bedpostX input
...
flags.DWI.runBedpostX = 1; % this takes ~10-20hrs per subject
